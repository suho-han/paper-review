...
% Optional (turn off by uncommenting disable):
...
% METADATA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\icmltitlerunning{Agentic Paper Reviewing}

\begin{document}

\twocolumn[
    \icmltitle{Agentic Paper Reviewing\\\vspace{0.15em}\large Academic Paper Reviewer Based on AI Agents}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
    % TODO (4--6 sentences recommended):
    % - Problem and motivation
    % - Key idea / approach
    % - Main results (quantitative where possible)
    % - Takeaway / impact
...
% - What is missing / limitations of prior work
% - Your contributions (bullet list is okay)
% - High-level overview of method and results
...
% - Any theoretical results (optional)

\subsection{Problem Setup}
Given an input manuscript (paper PDF or extracted text) and an optional target venue, the goal is to generate a structured review that includes (i) a summary of the work, (ii) strengths and weaknesses, (iii) actionable suggestions, and (iv) an overall rating and/or rubric-style sub-scores.
Our evaluation focuses on whether the system recovers salient weaknesses present in human reviews, how well its ratings correlate with human ratings when available, and how often generated claims are unsupported by the provided context.
...
In our default configuration, we run a lightweight model (e.g., Llama-3.2-1B-Instruct) and a stronger model (e.g., Llama-3-8B-Instruct) on separate ports with configurable GPU memory utilization and maximum context length.
...
\paragraph{Dataset.}
We evaluate on a curated test set derived from OpenReview, consisting of $N=899$ forum threads with publicly available human ratings.
For each thread, we use the mean of available human ratings as the reference score.
...
On a random sample of ICLR 2025 submissions, they report that the AI score achieves Spearman correlation comparable to human--human agreement and provides non-trivial acceptance prediction AUC (Table~\ref{tab:paperreviewai_reported}).
...
\begin{table}[t]
    \centering
    \caption{PaperReview.ai reported score-agreement metrics on ICLR 2025 submissions (from~\cite{paperreviewai}). These results are shown only for context and are not directly comparable to Table~\ref{tab:baseline_vs_agent}.}
...
Future work includes improving retrieval for non-arXiv domains, more reliable novelty assessment, and stronger faithfulness checks for factual claims.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 6. RELATED WORK (as requested)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Related Work}
% NOTE:
...
% TODO:
% - If you want: deeper positioning vs. a specific sub-community
% - Or: discuss additional concurrent work, ethics, limitations
\paragraph{Automatic peer review and scientific assistants.}
Recent work explores LLMs for scientific feedback and peer review, including agent-based analyses of peer-review dynamics~\cite{jin2024agentreview}, multi-agent discussion to generate more specific feedback~\cite{darcy2024marg}, and large-scale empirical studies of overlap between LLM feedback and human feedback~\cite{liang2024usefulfeedback}. These studies suggest that LLM feedback can be helpful but may exhibit systematic blind spots (e.g., weaker novelty assessment), motivating workflows that explicitly retrieve context and structure critique.
Our emphasis differs in that we target evidence-grounded reviewing behaviors by combining retrieval (for grounding) with agentic control (for verification and iterative refinement), rather than treating review generation as a purely generative task.
